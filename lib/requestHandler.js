...
function getUserReqHandler(userRule, recorder) {
  ...
  return function (req, userRes) {
    ...
    */
    ...
    const path = urlPattern.path.split("?")[0];
    ...
    if (path.match(/\.(png|jpg|jpeg|gif|svg|eot|mp3|json)$/i)) {
      let fullPath = "."

      const paths = `download/${host}/${path}`.split('/').filter(item => !!item)
      paths.forEach((item, index) => {
        ...
        if (index === paths.length - 1) {
          downloadFile(fullUrl, fullPath)
            .catch(err => {
              logUtil.printLog(`下载失败：${fullUrl}：${err}`)
            })
        } else {
